import{g}from"./background-43af18d8.js";const u={list:document.querySelector(".fav-list"),text:document.querySelector(".fav-text")},S=localStorage.getItem("localStorageData"),b=JSON.parse(S);let y=[],v=[],x=!1;u.list.addEventListener("click",D);q();function q(){if(!S)return;const t=Object.values(b);for(const e of t)y.push(e),x=!0}x&&(u.list.innerHTML=w(y),E(),h());function w(t){return t.map(({name:e,bodyPart:a,target:o,time:s,burnedCalories:n,_id:l})=>`
    ...
    `).join("")}function D(t){if(!t.target.closest(".fav-btn-delete"))return;async function e(){try{const s={endpoint:`exercises/${t.target.closest(".fav-item").dataset.id}`};return await g(s)}catch(o){console.log(o)}}e().then(({_id:o,bodyPart:s,name:n,target:l,burnedCalories:c,time:i})=>{const r={_id:o,name:n,burnedCalories:c,bodyPart:s,target:l,time:i},d=`Exercies-Name: ${n}`;let f=JSON.parse(localStorage.getItem("localStorageData")||"{}");localStorage.getItem(d)!==null?(delete f[d],localStorage.removeItem(d,JSON.stringify(r)),localStorage.setItem("localStorageData",JSON.stringify(f))):(f[nameOfEx]=r,localStorage.setItem(nameOfEx,JSON.stringify(r)),localStorage.setItem("localStorageData",JSON.stringify(f)))});const a=t.target.closest(".fav-item");a&&C(a),h()}function C(t){const e=t.id;document.getElementById(e).remove()}function h(){u.list.childElementCount===0?u.text.classList.remove("is-hidden"):u.text.classList.add("is-hidden")}function E(){v=document.querySelectorAll(".fav-btn-start"),v.forEach(t=>t.addEventListener("click",W))}const L=document.querySelector("[data-modal-close]"),m=document.querySelector(".modal-section"),k=document.querySelector(".btn-add-to-favorites"),O=document.querySelector(".modal-window"),$=document.querySelector(".name-exercies"),M=document.querySelector(".rating_value"),N=document.querySelector(".target"),B=document.querySelector(".body"),F=document.querySelector(".equipment"),J=document.querySelector(".popular"),A=document.querySelector(".description-of-exercises"),T=document.querySelector(".img-modal"),I=document.body;window.addEventListener("keydown",t=>{t.key==="Escape"&&p()});m.addEventListener("click",t=>{t.target===m&&p()});L.addEventListener("click",p);function p(){m.classList.toggle("is-hidden"),I.style.overflow="visible"}function W(t){t.preventDefault(),k.style.display="none",m.classList.toggle("is-hidden"),I.style.overflow="hidden";async function e(){try{const o={endpoint:`exercises/${t.currentTarget.closest(".fav-item").dataset.id}`};return await g(o)}catch(a){console.log(a)}}e().then(({_id:a,bodyPart:o,description:s,equipment:n,gifUrl:l,name:c,popularity:i,rating:r,target:d})=>{U(a,o,s,n,l,c,i,r,d)})}function U(t,e,a,o,s,n,l,c,i){O.setAttribute("data-id",t),$.textContent=n,M.textContent=Number(c),N.textContent=i,B.textContent=e,F.textContent=o,J.textContent=l,A.textContent=a,T.src=s}
